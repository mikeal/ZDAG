...
thoughts about how to explain how a developer could intentionally
...
What makes desinging with ZDAG so different is that is has compression
rules, and those compression rules are applied to deterministic aspects
of the structure you give it.

This means you can effectively program the compressor with your data
structure.

This means you can write a compression algorithm specific to your
application, pass the resulting data to ZDAG for compression, and
...
Whenever a list is all the same type it uses a differnet token to open
itself so that the list becomes a single series of TABLE indexes, removing
the type byte.

Since we already have to STR (125) tokens in this structure we can
...
[
  [    [    '20200827', '03:19:40.0'    ]  CID(1)      ],
/*
  118  118  0            0            126  117 8       126
  LIST LIST INDEX        INDEX   END_LIST  LINK INDEX  END_LIST
*/
]
  // ... and on and on for 244 more entries over many months
]
...
This is an important thing to recognize. One we have to entries in any
...
of bytes as just adding another string to the list.

A string/byte in an untyped list costs 1 + VARINT_INDEX. A typed
list removes the token bytes of both entries and only costs
a new termination token.
...
[
  [    [    '20200827',  '03',    '19:40.0'  ]           CID(1)     ],
/*
  118  118  245 2           0     32         126            117 8       126
  LIST LIST INDEX       INDEX  INDEX         END_LIST  LINK INDEX  END_LIST
*/
]
  // ... and on and on for 244 more entries over many months
]
...
The problem this algorithm would encounter trying to optimize this is balancing
...
This means that each chunk is costing us 2 bytes per chunk because every new
...
At some point the table can't be optimized.
...
and just chart the difference between each entry since it's only ever going
...
As you can see, this put a pretty big hold in the data structure.
...
Since the keys don't repeat and the values are all CIDs we
can get a typed map and save two bytes for every entry
because map keys don't need a typing token and a CID
typed map removes the type token for the value.
